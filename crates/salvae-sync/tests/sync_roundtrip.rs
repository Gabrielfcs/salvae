...
fn coop_session_round_trip_and_conflict() {
    ...
    let mut owner =
        SyncEngine::new(&channel, key, "Gabriel", "dev-gabriel", 5, owner_backups.path());
    assert!(matches!(
    ...
    let mut friend =
        SyncEngine::new(&channel, key, "Ana", "dev-ana", 5, friend_backups.path());
    assert!(matches!(
        friend.pull("valheim", friend_save.path(), 2_000).unwrap(),
        PullOutcome::Applied(v) if v.number == 1
    ));
    assert_eq!(std::fs::read(friend_save.path().join("world.db")).unwrap(), b"day 1 progress");
    ...
    assert_eq!(std::fs::read(owner_save.path().join("world.db")).unwrap(), b"day 2 progress");
}
